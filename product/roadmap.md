...
| Item | Source | Notes |
|------|--------|-------|
| ~~`@capacitor/cli` version bump~~ ✓ | observed | **RESOLVED 2026-05-31 (S155):** unblocked by upgrading the VM to Node 24 LTS (was Node 20; v8 CLI needs ≥22 — see DD-18). Bumped `^7.5.0` → `^8.1.0` (cli 8.3.4); `cap sync` + Vite build + 47/47 tests verified. |
...
| ~~Catalogue Capacitor origins, narrow CORS~~ ✓ | DD-16 watch-for | **RESOLVED 2026-05-31 (S155):** 7-origin allowlist landed in `api/src/index.ts` (Android native = `https://localhost` per Cap8 default scheme — corrected from the stale `http://localhost` catalogue). Verified on `:3002` (all allowed reflected, native preflight passes, evil origin blocked). See DD-16. Effective on `luby-api` restart. |
